...
(check-not-judgment-holds* (⊢p (desugar-program ((class Person (object) (method "__init__" self ((name str) (age int)) dynamic pass)) (define/assign p1 dynamic (Person "Alice" "21"))))))
...
(check-not-judgment-holds* (⊢p (desugar-program ((class C (object) (method "m" self ((x dynamic)) dynamic pass)) (def f () dynamic (begin (expr ((attribute (C) "m") 1 2))))))))

;; conformance_suite/methods_check_input_types.py
(check-not-judgment-holds* (⊢p (desugar-program ((class C (object) (method "m" self ((x int)) dynamic (return None))) (expr ((attribute (C) "m") "foo"))))))

;; conformance_suite/methods_check_output_types.py
(check-not-judgment-holds* (⊢p (desugar-program ((class C (object) (method "m" self () int (return "foo")))))))

;; conformance_suite/methods_work.py
(check-judgment-holds* (⊢p (desugar-program ((class C (object) (method "m" self ((x int)) str (return "foo"))) (define/assign s str ((attribute (C) "m") 42))))))
...
(check-not-judgment-holds* (⊢p (desugar-program ((class C (object) (field "x" int)) (class D (C) (method "x" self () dynamic pass))))))
...
(check-not-judgment-holds* (⊢p (desugar-program ((class C (object)) (class D (C)) (class A (object) (method "m" self ((x C)) None (return None))) (class B (A) (method "m" self ((x D)) None (return None)))))))

;; conformance_suite/override_instance_method_contravariant_inputs_pos.py
(check-judgment-holds* (⊢p (desugar-program ((class C (object)) (class D (C)) (class A (object) (method "m" self ((x D)) None (return None))) (class B (A) (method "m" self ((x C)) None (return None)))))))

;; conformance_suite/override_instance_method_covariant_output_neg.py
(check-not-judgment-holds* (⊢p (desugar-program ((class C (object)) (class D (C)) (class A (object) (method "m" self () D (return (C)))) (class B (A) (method "m" self () C (return (C))))))))

;; conformance_suite/override_instance_method_covariant_output_pos.py
(check-judgment-holds* (⊢p (desugar-program ((class C (object)) (class D (C)) (class A (object) (method "m" self () C (return (C)))) (class B (A) (method "m" self () D (return (D))))))))
...
(check-not-judgment-holds* (⊢p (desugar-program ((import-from "__static__" ("clen")) (def f ((l dynamic)) dynamic (begin (expr (clen l))))))))
...
(check-judgment-holds* (⊢p (desugar-program ((import-from "typing" ("Optional")) (class C (object) (method "__init__" self () dynamic (define/assign (attribute self "field") dynamic self))) (def g ((x C)) dynamic (begin pass)) (def f ((x (subscript Optional C)) (y (subscript Optional C))) dynamic (begin (if (is x None) (begin (define/assign x dynamic y) (if (is x None) (begin (return None)) (begin (return (g x))))) (begin (return (g x)))) (return None)))))))

;; conformance_suite/test_if_else_optional_return.py
(check-judgment-holds* (⊢p (desugar-program ((import-from "typing" ("Optional")) (class C (object) (method "__init__" self () dynamic (define/assign (attribute self "field") dynamic self))) (def f ((x (subscript Optional C))) dynamic (begin (if (is x None) (begin (return 0)) (begin)) (return (attribute x "field"))))))))
...
(check-judgment-holds* (⊢p (desugar-program ((import-from "typing" ("Optional")) (class C (object) (method "__init__" self () dynamic (define/assign (attribute self "field") dynamic 42))) (def f ((x (subscript Optional C))) dynamic (begin (if (is-not x None) (begin (return (attribute x "field"))) (begin)) (return None)))))))

;; conformance_suite/test_if_optional_cond.py
(check-judgment-holds* (⊢p (desugar-program ((import-from "typing" ("Optional")) (class C (object) (method "__init__" self () dynamic (define/assign (attribute self "field") dynamic 42))) (def f ((x (subscript Optional C))) dynamic (begin (return (if (is-not x None) (attribute x "field") None))))))))

;; conformance_suite/test_if_optional_dependent_conditions.py
(check-judgment-holds* (⊢p (desugar-program ((import-from "typing" ("Optional")) (class C (object) (method "__init__" self () dynamic (define/assign (attribute self "field") (subscript Optional C) None))) (def f ((x (subscript Optional C))) C (begin (if (bool-op and (is-not x None) (is-not (attribute x "field") None)) (begin (return x)) (begin)) (if (is x None) (begin (return (C))) (begin)) (return x)))))))

;; conformance_suite/test_incompat_override.py
(check-not-judgment-holds* (⊢p (desugar-program ((class C (object) (field "x" int)) (class D (C) (method "x" self () dynamic pass))))))

;; conformance_suite/test_incompat_override_method_arg_name.py
(check-not-judgment-holds* (⊢p (desugar-program ((class A (object) (method "m" self ((x str)) int (return 42))) (class B (A) (method "m" self ((y str)) int (return 0)))))))

;; conformance_suite/test_incompat_override_method_arg_type.py
(check-not-judgment-holds* (⊢p (desugar-program ((class A (object) (method "m" self ((x str)) int (return 42))) (class B (A) (method "m" self ((x int)) int (return 0)))))))

;; conformance_suite/test_incompat_override_method_arg_type_okay.py
(check-judgment-holds* (⊢p (desugar-program ((class A (object) (method "m" self ((x str)) int (return 42))) (class B (A) (method "m" self ((x object)) int (return 0)))))))

;; conformance_suite/test_incompat_override_method_num_args.py
(check-not-judgment-holds* (⊢p (desugar-program ((class A (object) (method "m" self () int (return 42))) (class B (A) (method "m" self ((x int)) int (return 0)))))))

;; conformance_suite/test_incompat_override_method_ret_type.py
(check-not-judgment-holds* (⊢p (desugar-program ((class A (object) (method "m" self () str (return "hello"))) (class B (A) (method "m" self () int (return 0)))))))

;; conformance_suite/test_inline_arg_type_mismatch.py
(check-not-judgment-holds* (⊢p (desugar-program ((import-from "__static__" ("inline")) (def f ((x int)) bool (begin (return (== x 1)))) (def g ((arg str)) bool (begin (return (f arg))))))))
...
(check-judgment-holds* (⊢p (desugar-program ((import-from "something" ("A" "B")) (class C (A B) (method "__init__" self () dynamic pass))))))
...
(check-judgment-holds* (⊢p (desugar-program ((import-from "typing" ("Optional")) (class C (object) (method "f" self ((x (subscript Optional "C"))) dynamic (if (is x None) (begin (return self)) (begin (define/assign p (subscript Optional "C") x)))))))))
...
(check-not-judgment-holds* (⊢p (desugar-program ((import-from "typing" ("Optional")) (def f ((a (subscript Optional int))) dynamic (begin (expr (unary-op - a))))))))
...
(check-not-judgment-holds* (⊢p (desugar-program ((def x ((a int) (b str)) None (begin pass)) (expr (x "a" 2))))))

;; conformance_suite/test_verify_positional_args_failure_method.py
(check-not-judgment-holds* (⊢p (desugar-program ((class C (object) (method "x" self ((a int) (b str)) None pass)) (expr ((attribute (C) "x") "a" 2))))))

;; conformance_suite/test_verify_positional_args_method.py
(check-judgment-holds* (⊢p (desugar-program ((class C (object) (method "x" self ((a int) (b str)) None pass)) (expr ((attribute (C) "x") 2 "hi"))))))
...
(check-not-judgment-holds* (⊢p (desugar-program ((def x () dynamic (begin (return 42))) (expr (x 1))))))
