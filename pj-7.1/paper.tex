...
Its static semantics for types is inspired by the Pyre~\cite{pyre} and mypy~\cite{mypy}
...
In fact, \SP{} has no plans to support all \PEP{} types.
...
They also narrow down via type tests as a kind of occurrence
...
If existing code uses the type $\sptset{\sptint}$ for sets of integers,
...
The behavior of the \SP{} dynamic type is subtle, however, because dynamic code
...
More to the point, \SP{} is less interested in guarantees about \emph{removing
...
but these traversals are bounded and typically short.
...
These constraints can catch bugs and always lead to more-efficient typed code.
Faster performance is not guaranteed, though, because concrete types must
check all writes from untyped contexts.
\Cref{t:shallow-vs-concrete} presents in more detail the worst-case costs of
...
If a value travels to untyped code, gets written to many times, and gets read few times,
then the shallow types may run faster overall.
...
typed classes can inherit from at most one other typed class.
...
Cinder vtables are specialized to check untyped overrides of typed methods
...
\SP{} accommodates this behavior provided that one parent is defined in typed
code and the untyped parents do not specify their members (via
\code{\_\_slots\_\_}).
Other uses of multiple inheritance must stay in untyped code.
...
timespan is therefore only a rough measure of their net effect.
...
\begin{enumerate}
  ...
  \item
    ...
    Rewriting these functions and call-sites to use positional arguments improves performance.
...
they do not require significant expertise to reproduce.
...
\code{time} utility).
Every benchmark version invoked the JIT on the same functions---even the
untyped versions.
...
slowdown.
This benchmark is primarily about number-crunching and the basic version uses standard
numeric types that \SP{} does not optimize.
Switching to primitive types without otherwise changing the code
...
Other \emph{optional\/} type checkers include Pyre~\cite{pyre},
mypy~\cite{mypy}, PyType~\cite{pytype}, and Pyright~\cite{pyright}; these
systems do not ensure safe interactions with untyped Python.
...
The current \CPUchange{} increase in CPU efficiency is a huge improvement
...
\acks{
  \SP{} was developed by Sinan Cepel, William Meehan, Carl Meyer, Aniket Panse,
  and Dino Viehland.
  ...
  Special thanks to Guido van Rossum for helping the authors meet via Twitter
  and for his ongoing contributions to Python.
}
...
\begin{table}[tp]
  ...
  \begin{tabular}{ll}
    ...
    () &  \\
    DeltaBlue \colname{T-Min-2} & \texttt{time ./python -X jit -X jit-list-file=Tools/benchmarks/jitlist\_deltablue\_static\_basic2.txt -X jit-enable-jit-list-w} \\
    ...
    DeltaBlue \colname{T-Min-2} & \texttt{time ./python -X install-strict-loader Tools/benchmarks/deltablue\_static\_basic2.py 100} \\
    (\colname{SP}) &  \\
    DeltaBlue \colname{T-Min-2} & \texttt{time ./python -X jit -X jit-list-file=Tools/benchmarks/jitlist\_deltablue\_static\_basic2.txt -X jit-enable-jit-list-w} \\
    ...
    DeltaBlue \colname{T-Min-2} & \texttt{time ./python Tools/benchmarks/deltablue\_static\_basic2.py 100} \\
    () &  \\
