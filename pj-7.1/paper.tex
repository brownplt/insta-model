...
%% TODO 2022-04-08 from Pyre team
%% Hi Carl! The small notational question I mentioned about in the paper was in
%% Figure 5 -- there's this "M-APP" rule that describes how classmethod invocation
%% is handled. I was trying to figure out how inherited classmethod would work
%% with this rule. Say I have a class B that defines a method f0(B, S0) -> S1, and
%% then I have another class D that inherits B which does not define additional
%% method. Now given Env0 \vdash expr0: S0, how can Env0 \vdash D.f0(expr0): S1 be
%% proven? If I understand correctly, the rule only looks up the type of f0 in
%% Env0(D), i.e. the body of the derived class. But the real definition of f0 is
%% syntactically located in the base class B. Does the notation system used here
%% assume that every function definition in base class gets automatically copied
%% into the derived class? Or should there be an additional preconditions added to
%% the "M-APP" rule saying that f0 is allowed to exist in any of the superclass of
%% D?

%% 2022-04-24 More model issues:
%% - C.f(e) uses a class type, not an instance ... how to invoke method on instance?!
%% ... we should really apologize for a too-simple model
%% - Object is not clearly in C
%% - introduce c again as variable name? gee why not expr.method()?
...
\begin{figure}[t]
  ...
  \\ & &
    ...
    %% TODO should C be a claass name? ... should c be variable name?
  \end{langarray}
...
%    }
  ...
  \begin{mathpar}
    ...
    \inferrule*{
      \sptclassdef{\sptclass_0}{\sptclass_1}{\ldots}{\ldots} \in \sptenv_0
    }{
