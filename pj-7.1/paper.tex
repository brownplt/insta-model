...
\newcommand{\numbenchmark}{2}
\newcommand{\numSPtest}{265}
\newcommand{\numSPtotal}{779} %% SP commit 6d61575 from Jan 9
\newcommand{\numSPdiff}{514} %% (\numSPtotal - \numSPtest)
...
and nominal classes.
Union types are tracked statically and narrow down in accordance
with type tests (a form of occurrence typing~\cite{tf-icfp-2010,gsk-esop-2011}),\footnote{
  For example, if \code{x} is an integer and \code{y} is a string, then the
  expression \code{x or y} has a union type.}
but are not generally supported at run-time~(\cref{s:eval-types}).
...
\begin{itemize}
  ...
  \item The others (\nnum{83}) test a wide range of other topics
