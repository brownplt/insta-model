...
\newcommand{\spchkdict}[1]{\spapp{\code{chkdict}}{#1}}
...
%% 2022-01-04 argument = nom soundness -> sp model sound -> sp sound via conformance equivalence
...
FILL pick names to avoid confusion between the Redex model and the tiny paper model?
...
%% TODO assume well-formed / normal-form
%% - no circular field defs (x:t = x)
%% - unions are tidy ... Opt[Dyn] = Dyn
%% kk

%% TODO surface typing judgment omitted
%% - straightforward
%% - eval typing far more interesting
%% - for now, main goal is to narrow unions ... that's kinda boring / not worth effort reading & writing
...
The basic values consist of the following: the none value, the top object, integers,
booleans (technically the integers \code{0} and \code{1}), and floating point numbers.
...
many details of Python including imports, conditionals, and exception handlers.
...
\subsection{Evaluation Types, Materialization, Casts}
...
\begin{figure}[t]
  ...
  \begin{mathpar}
    ...
    \inferrule*{
      \sptenv_0 \wtexpr \spexpr_0 : \sptype_0
    }{
      \sptenv_0 \wtprog \spvardef{\spx_0}{\sptype_0}{\spexpr_0} \dashv \sptvardef{\spx_0}{\sptype_0},~\sptenv_0
    }

    \inferrule*{
      \sptenv_1 = \sptfundef{\spf_0}{\sptype_0}{\sptype_1},~\sptenv_0
      \\\\
      \sptenv_1 \wtexpr \spexpr_0 : \sptype_1
    }{
      \sptenv_0 \wtprog \spfundef{\spf_0}{\spann{\spx_0}{\sptype_0}}{\sptype_1}{\spexpr_0} \dashv \sptenv_1
    }

    \inferrule*{
      \sptenv_1 = \sptclassdef{\spc_0}{\spc_1}{\sptvardef{\spx_0}{\sptype_0}}{\sptfundef{\spf_1}{\spc_0, \sptype_1}{\sptype_2}}, \sptenv_0
      \\\\
      %% TODO are field overrides exact?
      \mbox{if $\spann{\spx_0}{\sptype_0'} \in \spenvapp{\sptenv_1}{\spc_1}$ then $\sptype_0 = \sptype_0'$}
      \\\\
      %% TODO can T override dyn?
      \mbox{if $\sptfundef{\spf_1}{\spc_2, \sptype_1'}{\sptype_2'} \in \spenvapp{\sptenv_1}{\spc_1}$ then $\sptype_1' \spsubteq \sptype_1$ and $\sptype_2 \spsubteq \sptype_2'$}
      \\\\
      \sptenv_1 \wtexpr \spvardef{\spexpr_0} : \sptype_0
      \\
      \spann{\spself}{\spc_0}, \spann{\spx_1}{\sptype_1}, \sptenv_1 \wtexpr \spexpr_1 : \sptype_2
    }{
      \sptenv_0 \wtprog \spclassdef{\spc_0}{\spc_1}{\spvardef{\spx_0}{\sptype_0}{\spexpr_0}}{\spfundef{\spf_1}{\spself, \spann{\spx_1}{\sptype_1}}{\sptype_2}{\spexpr_1}} \dashv \sptenv_1
    }
  \end{mathpar}

  \fbox{$\sptenv \wtexpr \spexpr : \sptype$}~selected rules
  \begin{mathpar}
    \inferrule*{
      \sptenv_0 \wtexpr \spexpr_0 : \sptype_1
      \\
      \sptype_1 \spcompat \sptype_0
    }{
      \sptenv_0 \wtexpr \spexpr_0 : \sptype_0
    }
    ...
    \inferrule*{
      ...
      \sptenv_0 \wtexpr \spapp{\spf_0}{\spexpr_1} : \sptdyn
    }

    \inferrule*{
      \sptfundef{\spf_0}{\sptype_0}{\sptype_1} \in \sptenv_0
      \\\\
      \sptenv_0 \wtexpr \spexpr_1 : \sptype_0
    }{
      \sptenv_0 \wtexpr \spapp{\spf_0}{\spexpr_1} : \sptype_1
    }
    ...
    \inferrule*{
      \sptenv_0 \wtexpr \spexpr_0 : \sptchkdict{\sptype_0}{\sptype_1}
      \\\\
      \sptenv_0 \wtexpr \spexpr_1 : \sptype_0
    }{
      \sptenv_0 \wtexpr \spdictref{\spexpr_0}{\spexpr_1} : \sptnone
    }

    \inferrule*{
      \spann{\spx_0}{\sptype_0} \in \spenvapp{\sptenv_0}{\spc_0}
    }{
      \sptenv_0 \wtexpr \spobjref{\spc_0}{\spx_0} : \sptype_0
    }
    ...
    \inferrule*{
      \sptfundef{\spf_0}{\spc_0, \sptype_0}{\sptype_1} \in \spenvapp{\sptenv_0}{\spc_0}
      \\\\
      \sptenv_0 \wtexpr \spexpr_0 : \sptype_0
    }{
      \sptenv_0 \wtexpr \spobjapp{\spc_0}{\spf_0}{\spexpr_0} : \sptype_1
    }

  \end{mathpar}

  \caption{Evaluation Typing Judgment}
  \label{f:eval-typing}
\end{figure}
...
\begin{figure}[t]
  \begin{langarray}
    \spvalue & \defeq &
      \mbox{heap-allocated Python values}
    \\
    \spteval & \defeq &
      \sptdyn \langmid
      \sptnone \langmid
  ...
  \fbox{$\sptype \spsubteq \sptype$} reflexive, transitive closure of $\spsubt$, defined below:
  \begin{mathpar}
    \inferrule*{
    }{
      \sptype_0 \spsubt \sptobject
    }
    ...
    \inferrule*{
      \sptype_0 \spsubt \sptype_1
    }{
      \sptype_0 \spsubt \sptoptional{\sptype_1}
    }

%    \inferrule*{
%    }{
%      \sptype_0 \spsubt \sptype_0
%    }
%
%    \inferrule*{
%      \sptype_0 \spsubt \sptype_1
%      \\
%      \sptype_1 \spsubt \sptype_2
%    }{
%      \sptype_0 \spsubt \sptype_2
%    }
  \end{mathpar}

  \fbox{$\sptype \spcompat \sptype$}
  \begin{mathpar}
    \inferrule*{
      \sptype_0 \spsubt \sptype_1
    }{
      \sptype_0 \spcompat \sptype_1
    }

    \inferrule*{
    }{
      \sptype_0 \spcompat \sptdyn
    }
  \end{mathpar}

  \fbox{$\sptype \spmatr \sptype$}
  \begin{mathpar}
    \inferrule*{
    }{
      \sptdyn \spmatr \sptype_0
    }
  \end{mathpar}

  \caption{TBA Type Metafunctions}
  \label{f:surface-to-eval-types}
\end{figure}

\begin{table}
  \caption{TBD Run-time checks}
  \label{t:cast}
  ...
  \(\begin{array}{l@{\quad}l}
    ...
    \sptobject & \mbox{Accepts any non-primitive value} \\  %% except primitives
    ...
    \sptbool & \mbox{Accepts booleans (\code{0} and \code{1})} \\
...
%% "full soundness" requires strict modules, means you can trust module toplevel
%% hmph
...
%% 2021-01-04 TODO have no evidence that "any module" typing works out ... team
