...
%% - threats to validity: only 1 migration path, no systematic evaluation (lattice)
...
\begin{itemize}
  \item
    ...
    With fine-tuned types, \SP{} consistently out-performs the Python baseline~(avg. \pctnum{67}).
...
recursive types.
Pyre and mypy offer some support for the first two kinds, but not for recursive types.
...
First off, the casts for non-generic nominal types are straightforward.
Every type has a name and every value has a tag that corresponds to the name.
A cast must examine the parents of a class to find a match in the worst case,
...
The \SP{} solution is provide a second kind of dictionary that ensures
the validity of its elements.
These two dicts have distinct types:
\begin{enumerate}
  \item
...
or leave it as-is with shallow types.\footnote{Not all generic \SP{} types have shallow
  and concrete versions at present.
  In particular, user-defined classes do not have concrete support.
  The shallow check for a user-defined class does, however, guarantee the types of all
...
multiple inheritance among typed classes (or untyped classes with
a typed ancestor) is forbidden.
...
although types are needed to enable the most-effective optimizations.
...
As noted in~\cref{s:type-dynamic}, this exact-match rules applies even when $\spteval_0$
...
In other words, \SP{} provides \emph{immediate accountability} same as
...
the Cinder runtime, which has instructions to check types at run-time, to
construct \SP{} data structures, and to perform actions in a way that benefits
...
Digging deeper, there are nine common kinds of changes: five address
type errors and the other four are for performance.
...
%% \textbf{Note to Programming Reviewers:}
%%  We plan to add a fourth benchmark to \cref{t:microbenchmark}.
...
Mixed configurations may reveal performance bugs similar to those found
by an evaluation of Retifulated Python~\cite{gm-pepm-2018}.

%% Although mixed configurations are unlikely to have disastrous
%% overhead~\cite{tfgnvf-popl-2016} due to \SP's efficient strategy for enforcing
%% types and Instagram's positive experience with mixed-typed code in
%% production~(\cref{s:migration-path}), they could reveal performance bugs
%% similar to those found by a prior evaluation of Retifulated
%% Python~\cite{gm-pepm-2018}.
...
A careful evaluation~\cite{gtnffvf-jfp-2019} of a few small projects would be
especially useful to find performance bottlenecks.
Second, the \SP{} approach should be adapted to other languages.
An optionally language such as TypeScript would be an ideal starting
point.
