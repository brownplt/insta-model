...
both of which support efficient run-time checks.
...
The Instagram codebase uses lots of classes and objects.
Most of these are simple, Java-style: classes declared on
the top level and single inheritance.
It was therefore essential to get inheritance working across type boundaries.
Typed classes must be able to inherit from untyped classes,
and vice-versa untyped classes must be able to inherit from typed ones.

FILL literature? StrongScript Nom classic GT?

FILL either way, spell out what happens for these interactions.
Where are the checks needed, checks skipped.
...
\subsubsection{Non-Highlights / Restrictions}

The \SP{} type system excludes include some features.
Each of the following is low priority for the team; that said, they are
ordered below rougly from most important (soonest to be implemented) to least.

\begin{itemize}
  \item \emph{Concrete Parameterized Classes}.
    Except for a handful of built-in types,
    parameterized classes are enforced only with shallow checks.
    In the near future, these classes will gain concrete versions
    for programmers to opt-in to.

  \item \emph{Callable Types, First-Class Classes}.
    The Instagram codebase rarely uses first-class functions, classes, or objects.
    Thus there is little need for either a \code{Callable} type to describe functions
    or first-class types for objects.
    When such values do appear, \SP{} assigns the dynamic type and spot-checks their
    uses.
    For example, every application of a first-class function gets wrapped in a check
    if the context expects a certain type.

  \item \emph{Method Overloading}.
    Methods cannot be overloaded.
    Each combination of a class and a name can refer to at most one method.
    This behavior comes from Python.

  \item \emph{Multiple Inheritance}.
    Multiple inheritance is not supported in a typed way.
    \Cref{s:impl} discusses this point further.

  \item \emph{Strong overrides}.
    Methods and fields may be overridden, but only with values of the same type.
  ...
  \item \emph{Recursive Types}.
    \SP{} does not support recursive types.
    Neither does Pyre or mypy.
...
%    Each instance of a \code{ChkList[int]} comes with the full type as a tag;
%    run-time checks inspect this tag for an exact match.\footnote{Immutable
%    data structures could use a subtype check. \SP{} does not have any of these
%    because the common Python data structures are all mutable.}
