...
The exceptions are the simply-typed version of \bmname{fannkuch} and
\bmname{nbody}.
Both these benchmarks focus on numeric operations, which \SP{} does not
optimize because it offers primitive types.
Indeed, changing \bmname{fannkuch} to use a few primitive types without
otherwise changing the code gives an immediate speedup (ratio = $0.90$).
