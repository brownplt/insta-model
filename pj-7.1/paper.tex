...
Flatt et al.~\cite{fkf-popl-1998} propose a different method that accommodates mixins,
but the method calls a second layer of interface types.
Muehlboeck and Tate~\cite{mt-oopsla-2021} similarly allow structural objects with interfaces.
...
Classes can inherit from any number of other classes.
To resolve method calls, Python traverses the class hierarchy
to find the first matching receiver (MRO).

[FILL]
\SP{} lets most programs use multiple inheritance.
The one constraint is that a class cannot have two
values bound to the same member name, to ensure v-table
integrity.
